...
export default (C) => {
  return class ActionMain extends C {
    ...
    load(fileNames) {
      ...
      this.setUserData(
        "recentProjectPaths",
        recentProjectPaths || [projectPath]
      );
      ...
      if (this.isLoaded) {
        try {
          ...
          this.parseQMap(
            JSON.parse(fs.readFileSync(file2Path, "utf8")),
            dataPath
          );
        } catch (e) {
          console.error(e);
    ...
    checkForQSprite() {
      ...
      try {
        ...
        if (this.hasQSprite) {
          ...
          this.QSprite.configs = JSON.parse(
            fs.readFileSync(qSpritePath, "utf8")
          );
        }
      } catch (e) {
        this.hasQSprite = false;
    ...
    changeTheme(theme) {
      ...
      if (themeElements.length === disabled) {
        console.warn(
          `Attempted to apply a theme that isn't registered: ${theme}`
        );
