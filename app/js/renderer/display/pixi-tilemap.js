...
class RectTileLayer extends Container {
  constructor(zIndex, texture) {
    ...
    this._$_localBounds = new Bounds();
    ...
    for (let i = 0, n = points.length; i < n; i += POINT_STRUCT_SIZE) {
      ...
      if (textureIndex >= 0) {
        renderer.context.drawImage(
          this.textures[textureIndex].baseTexture.getDrawableSource(),
          x1,
          y1,
          w,
          h,
...
class CompositeRectTileLayer extends Container {
  ...
  initialize(zIndex, bitmaps, texPerChild) {
    ...
    this.texPerChild =
      texPerChild || Constant.boundCountPerBuffer * Constant.maxTextures;
    if (bitmaps) {
  ...
  setBitmaps(bitmaps) {
    for (let i = 0; i < bitmaps.length; i++) {
      if (bitmaps[i] && !bitmaps[i].baseTexture) {
        throw new Error(
          `pixi-tilemap cannot use destroyed textures. ` +
...
class GraphicsLayer extends Graphics {
  constructor(zIndex) {
    super();
    this.zIndex = zIndex;
  }
...
class MultiTextureResource extends resources.Resource {
  constructor(options) {
...
class RectTileGeom extends Geometry {
  constructor() {
    super();
    this.vertSize = 11;
    this.vertPerQuad = 4;
...
class ZLayer extends Container {
