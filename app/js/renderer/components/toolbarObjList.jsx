...
export default class ToolbarObjList extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      nextProps.currentMap !== this.props.currentMap ||
  ...
  componentDidUpdate(prevProps) {
    if (
      this.props.currentMap > 0 &&
      prevProps.currentMapObj !== this.props.currentMapObj
    ) {
      const selected = document
        .getElementById("ListMapObj")
  ...
  body() {
    ...
    return (
      <div style={{ height: "100%" }}>
        <div className="header">Map Objects</div>
        <SortableList
          items={mapObjects}
  ...
  render() {
    const { currentMap, currentMapObj } = this.props;
